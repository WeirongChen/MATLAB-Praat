...
%                Oct-2021: Added tier selection
%                Mar-2022: Embeded supporting functions within the main function. 
...
% Check if 'whichTier' doesn't exist in the TextGrid
totalTierInds = 1:numel(tierNames);
if isempty(tierInd) || any(~ismember(tierInd, totalTierInds))
...
%% Supporting sub-functions:
        ...
        function [segs, labs]=ParseOneTier(lines, isShort, isPoint, tierStart)
            ...
            for ilab = 1:numel(labs)
                lab = labs{ilab};  labs{ilab} = strrep(lab, '""', '"');
            end
        ...
        % get tier names and types
	        ...
	        ind1 = find(~cellfun(@isempty,regexp(lines,'TextTier')));
            if ~isempty(ind1)
		        pointTiers = ind1 + 1;
	        ...
	        ind1 = find(~cellfun(@isempty,regexp(lines,'IntervalTier')));
            if ~isempty(ind1)
		        intTiers = ind1 + 1;
        ...
        function outTextGridLines=DetectStrainedQuotationSybol(TextGridLines)
            ...
            ToDelete=[];
            for iii = 1:length(TextGridLines)-1
                thisLine = TextGridLines{iii};
                nextLine = TextGridLines{iii+1};
                thisLine = strrep(thisLine,' ','');
                nextLine = strrep(nextLine,' ','');
                if strcmp(thisLine,'text="') && strcmp(nextLine(end),'"')
                    ToDelete = [ToDelete iii+1]; 
                    outTextGridLines{iii} = [TextGridLines{iii} TextGridLines{iii+1}];
                end
            end
        ...
        % Usage: encoding = DetectTextGridEncoding(TextGridFName)
            [~,~,ext]=fileparts(TextGridFName);
            if isempty(ext), TextGridFName=[TextGridFName '.TextGrid'];end
            ...
            warning('off',wid);
            for jjj=1:length(encodings)
                fid = fopen(TextGridFName, 'r', 'l', encodings{jjj});
                S = fscanf(fid, '%c'); fclose(fid);
                out = strfind(S, 'Text'); encodingWeight(jjj)=length(out);
            end
