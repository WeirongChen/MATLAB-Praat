function Write2TextGrid(TG, outfilename,TotalDur)
% Usage: Write2TextGrid(TG, outfilename, TotalDur)
...
errorFlag = 0; 
for i = 1:numel(TG)
    if hasfield(TG(i), 'IsPointTier'), isPoint = TG(i).IsPointTier; else, isPoint = size(TG(i).segs,2) ==1; end
    if hasfield(TG(i), 'xmin'), xmin = TG(i).xmin;  else, xmin = 0; end
    if hasfield(TG(i), 'xmax'), xmax = TG(i).xmax;  else, xmax = Dur; end
